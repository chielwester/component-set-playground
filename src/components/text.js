(() => ({
  ...
  jsx: (() => {
    ...
    // const {useText} = B;
		...
		const jsonItem = parent && parent.jsonItem ? parent.jsonItem : null;

		const dataattribute = options.dataattribute;
		const dataattribute_value = B.useText(options.dataattribute);

    return (
      <Tag className={classes.content}>
				{jsonItem && B.env === 'prod' ? eval(options.content[0]) : ''}
