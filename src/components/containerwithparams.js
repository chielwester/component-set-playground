(() => ({
  ...
  category: 'LAYOUT',
  type: 'ROW',
  allowedTypes: ['COLUMN', 'ROW', 'TEXT', 'TITLE', 'PANEL', 'GOOGLEMAP', 'FORM', 'DATATABLE'],
  orientation: 'HORIZONTAL',
  jsx: (
    <div className={classes.container}>
    {(() => {
			// const {useParams} = B;
			const {Children, GetOne} = B;
					...
					{(() => {
						if(B.env == 'prod') {
							...
							if(options.variablename) {
								const variablevalue = parseInt(params[options.variablename]);

								return (<GetOne modelId={options.model} byId={variablevalue}>
									{({loading, error, data}) => {
										if(loading) {
							...
							return isPristine ? 'Row' : children;
						} else {
							return isPristine ? 'Row' : children;
						}
